...
-- ═══════════════════════════════════════════════════════════════════════════
--
-- 024 is mostly additive: seven new `core` functions, no DDL, no function it
-- replaced — plus one DATA patch (§0 of the forward migration), a
-- TRAINER/attendance:export row in `app.role_permissions` that 002 (frozen)
-- does not carry. The rollback is therefore seven `DROP FUNCTION` plus that
-- one `DELETE`, nothing else to restore. Run against a database with 024
-- applied and nothing later.
--
...
DELETE FROM app.role_permissions WHERE role = 'TRAINER' AND permission = 'attendance:export';
...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM app.role_permissions WHERE role = 'TRAINER' AND permission = 'attendance:export') THEN
    RAISE EXCEPTION '024 rollback verify: TRAINER still holds attendance:export';
  END IF;
END
$verify$;
