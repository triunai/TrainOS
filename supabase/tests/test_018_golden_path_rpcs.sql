...
-- NO HAND-MADE PIPELINE FIXTURE. **019** seeds every tenant two default
-- pipelines and sixteen steps from `trg_tenants_z_seed_pipelines`, so the
-- tenant INSERTs above already provisioned them — and inserting a second
-- `is_default` ENGAGEMENT row here would violate `pipelines_one_default_uq`.
--
-- ⚠ THIS PIN THEREFORE REQUIRES 019 TO BE APPLIED. `core.navigation` and
-- `core.get_pipeline_config` render stages FROM `core.pipeline_steps` and
-- inline no stage list, which is the rule — so without 019 both return an empty
-- stage list and T9 and T10 fail for a reason that is not 018's. That is this
-- repo's stated testing philosophy rather than an exception to it: "pins are
-- executed against the FULL applied set, not immediately after their own
-- migration". The seed itself is pinned by `test_019`.
--
