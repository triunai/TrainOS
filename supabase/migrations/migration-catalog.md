...
| `core.reveal_provider(text)` | authenticated; `ai:provider:reveal` + `app.aal2_verified()` | `POST .../{id}/reveal` -> `{key, revealedAt}` |
| `core.delete_provider(text)` | authenticated; `ai:provider:delete` + aal2 claim | `DELETE .../{id}`, `AGENT_PAUSED {blockers}` |
| `app.provider_key_for_tenant(uuid,text)` | **service_role only** | worker BYOK read; RUN_CALL audit per key |
| `app.provider_key_salts`, `app.provider_key_probes` | none; RLS forced, no policy | salt for HMAC fingerprint; probe outcome, no body |

All `ai:provider:*` permissions are ADMIN-only in 002:1134-1139; MD holds none, so the four hosted founders (all MD) are FORBIDDEN on the Providers screen until they hold ADMIN or 002's matrix changes. Reported, not changed here.

**Evidence.** Pin T3/T12 grep every text-like column of every core/app/public table for each raw key (text and hex): 0 hits in 733 columns, before and after every path. T4 proves refusals (shape, duplicate, unsupported provider, a forced internal error) carry no 6-character window of the key. T7 proves aal1 and a forged aal2 claim are refused, the aal2 reveal writes its REVEAL audit and event, both rate limits trip, and 013's `REVEAL_AUDIT_REQUIRED` trigger still refuses a bare bump. T8 records synthetic `net._http_response` rows (the shim pg_net has no worker) through every classification. Teeth: removing the reveal AAL2 gate fails T7a; removing the opaque re-raise fails T4g. Rollback diffed against a fresh 001–021 build with `pg_dump --schema-only`: empty. On the shim, `vault` is a stand-in with supabase_vault 0.3.x's SQL surface and Supabase's grants, and a pgcrypto cipher.

**Stated, not closed** (029 header L1–L3). RPC arguments reach Postgres logs only under `log_statement` all/mod, a met `log_min_duration_statement`, `log_parameter_max_length_on_error > 0`, or pgaudit parameter logging; hosted settings unverified. `vault.create_secret` INSERTs plaintext before encrypting, so the key is in WAL. pg_net holds the key in `net.http_request_queue.headers` until its worker sends the request, and upstream pg_net grants ALL on `net` tables to PUBLIC. `core.ai_provider` (003:107) lacks the contract's `OPENROUTER`/`OTHER`, which are refused as `UNSUPPORTED_PROVIDER`. 027's `core.list_providers` must filter `key_ref NOT LIKE 'retired:%'` to hide tombstoned keys.
