...
| # | File | Summary |
|---|------|---------|
| 020 | `020_api_read_surface.sql` | **The API read surface, part 1: approvals that can be decided (2026-09-14).** Three 018 functions replaced (`core.list_approvals`, `core.get_approval`, `core.get_audit`). Each body is 018's except for the blocks marked `-- 020 ·`. No table, no enum value, no policy. **Nobody could approve anything**: 014's `core.decide_approval` requires the diff hash and 011 compares it, but 018's list and detail never emitted `diffHash`, so every APPROVE was refused `VALIDATION_FAILED`. Both projections now carry it, as the contract on origin/main requires (`ApprovalRequest.diffHash`). **The audit drawer was always empty**: the client sends the URL resource segment (`approvals`), and 012 stores UPPER_SNAKE aggregate types. `get_audit` maps the unambiguous segments and still accepts UPPER_SNAKE, and for `approvals` it returns the trail correlated to the approval's action request. **The three reads gain the permission gate** `list_quotations` already had (`approval:read`, `audit:read`), decided before any read. `decide_approval` keeps 014's uuid signature; the web passes the id. Pin `test_020` runs every probe as `authenticated` against a real JWT principal, end to end through an APPROVE, on a shim where every migration ran as a NOSUPERUSER BYPASSRLS role. |
...
| Object | Change | Why |
|---|---|---|
| `core.list_approvals` | `'diffHash', a.diff_hash` in each row; `approval:read` gate first | 014 refuses an APPROVE with no hash, and 018 never sent one |
...
The signatures are 018's exactly, and `$verify$` V1 asserts one definition each.
`core.decide_approval(uuid, …)` (014) is not redefined, and V4 asserts that. Segments that
address more than one aggregate (`hrdc`, `collections`, `compliance`, `knowledge`) are deliberately
not mapped.
...
T1 a SALES `PROPOSAL_SEND` queues and the SALES_MANAGER's list row and detail carry `diffHash`
equal to the stored hash. T2 APPROVE with the uuid and the listed hash succeeds and the proposal
is SENT; a stale hash is refused `DIFF_CHANGED` with the fresh hash in details; no hash is refused
`VALIDATION_FAILED`, and the approval is unchanged. T3 CLIENT is FORBIDDEN on list, detail and
audit, identically for a real and an invented ref. T4 another tenant's MD gets zero rows,
NOT_FOUND and an empty trail. T5 `approvals::{ref}` returns the correlated event,
`proposals::{ref}` maps to `PROPOSAL`, the UPPER_SNAKE spelling answers the same. T6 anon is
refused 42501. Every probe runs as `authenticated` and is one statement. Measured red on 001–019
without 020 (T1c).

### Rollback — `rollbacks/020_api_read_surface_rollback.sql`

Re-creates the three 018 definitions in full and restates 018's grants, all in one transaction.
Measured: a catalog snapshot of functions, views, columns, ACLs, policies, triggers,
constraints and seed-row counts is identical for 001–019 and for 001–019 + 020 + rollback.
