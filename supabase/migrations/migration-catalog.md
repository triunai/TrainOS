...
executor itself. T4b: in a tenant with only MD users (akademi-perdana's shape), OPP-01 routes to the other MD, the requester is refused GOV-03, and the other MD approves. **Mutation-tested**, each red when its guard is reverted:
the executor re-check removed (T4k), one RPC gate removed (T1a), 020's `get_audit` restored (T3b).
...
It **refuses** if any request, approval,
...
restores 011's three functions, 020's `get_audit` and the 24 018 bodies in full, with their
