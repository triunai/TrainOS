...
**Migrations:** 22 (021 is a sibling lane authoring `lane/api-021`, not present on this branch) · **Applied (hosted):** 001–019 · **Authored, not applied:** 020, 022
...
| # | File | Summary |
|---|------|---------|
| 022 | `022_profile_and_dashboard.sql` | **`core.me_profile` and the two executive-dashboard RPCs 020 reported NOT BUILT, built from real data (2026-09-14).** Three new `core` functions, nothing replaced, nothing in the spine touched. `core.get_hours_saved` and `ADMIN_HOURS_SAVED` are deliberately not built — no baseline-minutes table exists. `MeProfile.location`/`jobTitle`/`department`/`staffNumber` are `null` (no table carries them); `session.browser`/`place` are `null` (no user-agent/geoip storage); `session.activeSessions`/`lastSignInAt`/`twoFactorEnabled` are real off `auth.sessions`/`auth.users.last_sign_in_at`/`auth.mfa_factors` (the last two guarded and degrading to `null` on any environment, this shim included, where the hosted-only GoTrue object is absent). Every other field, including `moduleCount` (the same nav-permission match `core.navigation()` uses, MD reads 13 of 14) and `email` (from `auth.users`), is real. The four dashboard metrics are each one worked aggregate over `core.opportunities`/`invoices`/`proposals`/`hrdc_packets`, formulas in the migration header; `agentActivity`/`autonomyMix` derive per-run autonomy from `core.action_requests.granted_level`; `agentSpend` sums `core.budget_status` at `scope='AGENT'` (no TENANT scope exists). No `delta` anywhere — no snapshot table exists to compute one honestly. Both dashboard RPCs gate on the already-seeded `dashboard:executive:read` (SALES_MANAGER/FINANCE/MD/ADMIN), narrower than `dashboard:read` and the closer match to the contract's `roles:['MD']`; `me_profile` gates on membership only. Pin `test_022`: 54/54 assertions against a hosted-like shim, hand-computed sums against seeded fixtures walked through `core.state_transitions`' gated edges (not gone around), tenant isolation, permission refusal, anon 42501, and validation on both RPCs. `test_001`–`test_020` unmodified and still green. `lint:sql`/`check:grants`/`check:rpc` clean. **Owed:** the skill's G5 dual adversarial review, flagged to `main` before merge. |
...
to any hosted database.** The shim ran 001–020 as a `NOSUPERUSER BYPASSRLS` role standing in for
hosted `postgres`, then 022 on top; 021 is a sibling lane authoring `lane/api-021` and is not
...
- **`session` (`ProfileSession`)** — `browser`/`place` are always `null` (no user-agent or geoip storage anywhere in this schema). `lastSignInAt` = `auth.users.last_sign_in_at`, read defensively (a caught `undefined_column` degrades to `null`) since this migration has no hosted access to confirm the column exists — it is a standard GoTrue column on every real Supabase project. `activeSessions` = `COUNT(auth.sessions)` for the caller, confirmed definer-readable. `twoFactorEnabled` = `EXISTS(auth.mfa_factors WHERE user_id = caller AND status = 'verified')`, guarded on `pg_catalog.to_regclass('auth.mfa_factors')` and `null` when the relation is absent — present on hosted Supabase, absent from this local shim, so the guard is the only way to prove the function does not hard-fail in either environment. Second ruling, prompted by `web-022`'s question about which `ProfileSession` fields are actually derivable.
...
54/54 assertions pass. T1: MD's `me_profile` carries exactly the 9 `MeProfile` keys (no `mobile`),
...
`compliance:read`, only the three scoped `compliance:*` strings). `session.browser`/`place` are
`null`; `session.activeSessions` = 2 against two seeded `auth.sessions` rows (a real, non-constant
count); `session.lastSignInAt`/`twoFactorEnabled` are `null` in this shim specifically because it
has no `last_sign_in_at` column and no `auth.mfa_factors` table — both real GoTrue objects on hosted
Supabase, and both derivation paths (null-degrade and real-value) were verified ad hoc against a
locally-extended copy of the shim's `auth` schema, not checked into this pin. T2: an AGENT principal
is `FORBIDDEN`; anon is refused
...
**Regression check:** `test_001`-`test_020` re-run against the same 001-020+022 build, unmodified,
all still green (no `FAIL` notice in any log; the naive `grep NOTICE.*FAIL` false-positives on
notice text like `VALIDATION_FAILED` were checked by hand). `npm run lint:sql` 67/68 (the one
failure, `supabase/seeds/test_hosted_demo.sql`, is a pre-existing file of psql meta-commands
unrelated to this migration — nothing else can parse it either). `npm run check:grants` clean.
...
- **G5 (the skill's dual adversarial review — thermonuclear + Codex trace) is OWED, not run.** The
  lane brief's own Proof section (lint:sql / check:grants / check:rpc / the pin, executed) did not
  ask for it, and a Sonnet-tier lane's time did not stretch to it. Flagged to `main` before merge.
