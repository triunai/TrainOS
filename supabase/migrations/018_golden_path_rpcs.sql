...
-- `security_invoker` views, fourteen internal `app._*` helpers and one
-- `app.event_subscriptions` routing row. 48 objects. IT CREATES NO TABLE AND NO
-- ENUM VALUE, and no existing table, function, view, policy or grant is
-- modified. Nothing in 001-017 is touched. The three gate wrappers call 011 and
-- add nothing.
--
-- ⚠ THE PER-TENANT PIPELINE SEED LEFT THIS FILE AND IS NOW 019. It was here —
-- a trigger on `public.tenants`, a cross-tenant backfill, a ledger table and a
-- reversal function — and the review was right that it did not belong. A
-- repo-wide semantic change does not ship as a subsection of a file whose
-- stated subject is reads and writes, and the cost was measurable: it turned
-- `test_008`, `test_009`, `test_016` and `test_017` red. It is now
-- `019_pipeline_provisioning.sql`, beside 016's ref formats and 017's check
-- keys, where its irreversibility is the subject of review rather than a
-- footnote.
--
-- ⚠ AND THIS PACK NOW DEPENDS ON 019. `core.navigation` and
-- `core.get_pipeline_config` render stages FROM `core.pipeline_steps` and
-- inline no stage list — that is the rule, and it means a tenant with no rows
-- gets an empty stage list from both. 018 applies and verifies cleanly without
-- 019; what needs 019 is a tenant having a lifecycle to render. `test_018` is
-- run against 001-019, which is this repo's stated testing philosophy
-- ("pins are executed against the FULL applied set") rather than an exception
-- to it.
--
...
--
-- 014, 015, 016 and 017 all wrap, forward and rollback, and 018 did not. It is
-- pure DDL plus one routing row now that the pipeline seed has moved to 019, so
-- the stakes are lower than they were — but a 5,000-line file that creates 48
-- objects and fails at object 31 leaves a state no rollback file describes,
-- because the rollback describes the finished migration.
--
...
BEGIN
  ...
  -- V16b/V16c MOVED TO 019. They asserted that the pipeline seed trigger
  -- exists on `public.tenants` and sorts alphabetically after 016's
  -- ref-format seed; both are properties of the seed, and the seed is 019's.
  -- A verify block that checks another migration's object passes vacuously
  -- when that migration has not been applied, which is the R4 defect this pack
