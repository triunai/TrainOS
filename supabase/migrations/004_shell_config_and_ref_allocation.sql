...
-- 002 and 003 on top, so 002's five public tables were born with all three
-- roles holding ALL. 014's verify refuses exactly that. This section brings
-- such a database to the state a fresh install reaches, and is a no-op on one
-- that already is.
-- It sits at the head of 004 rather than 002 or 003 because hosted already
-- held 001-003 when it was written; 003 creates nothing in public.
--
...
BEGIN
  ...
  IF v_extra IS NOT NULL THEN
    RAISE EXCEPTION '004 hosted remediation: public holds objects 002 did not create: %. '
      'This section is written for the state directly after 002-003 and will not guess '
  ...
  CREATE TEMPORARY TABLE t004_api_acl_target (
    relname text NOT NULL, grantee text NOT NULL, privilege text NOT NULL
  ) ON COMMIT DROP;
  INSERT INTO t004_api_acl_target (relname, grantee, privilege) VALUES
  ...
  FOR v_rel IN
    SELECT DISTINCT excess.relname, excess.grantee,
           (SELECT pg_catalog.string_agg(target.privilege, ', ')
              FROM t004_api_acl_target AS target
      ...
      FROM (SELECT class.relname::text AS relname,
            ...
            EXCEPT
            SELECT relname, grantee, privilege FROM t004_api_acl_target) AS excess
  LOOP
  ...
  SELECT
    (SELECT pg_catalog.string_agg(pg_catalog.concat_ws('.', relname, attname) || ':'
       ...
       FROM (SELECT relname, attname, grantee, privilege FROM api
             EXCEPT
             SELECT relname, NULL::text, grantee, privilege FROM t004_api_acl_target) AS extra),
    (SELECT pg_catalog.string_agg(relname || ':' || grantee || ':' || privilege, ', ')
       FROM (SELECT relname, grantee, privilege FROM t004_api_acl_target
             EXCEPT
  ...
  IF v_leftover IS NOT NULL OR v_missing IS NOT NULL THEN
    RAISE EXCEPTION '004 hosted remediation: public ACLs for the API roles differ from '
