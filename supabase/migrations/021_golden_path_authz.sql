...
-- body byte for byte, apart from the blocks marked `-- 021 ·`. The rollback
-- reproduces the 018 bodies in full.
--
-- ── DEFECT · A PRINCIPAL WITH A TENANT CLAIM COULD CALL EVERY READ AND WRITE ─
--
-- Of 018's thirty RPCs, only `core.list_quotations` checked a permission
-- (018:4502). 020 gated `list_approvals`, `get_approval` and `get_audit`. The
-- remaining 23 checked nothing, or only `current_actor().role IS NULL`, so a
-- TRAINER or CLIENT principal with a tenant claim could read enquiries and
-- quotations and write proposals. 002 §11 holds the matrix as data, and
-- `app.has_permission` is its reader.
--
-- ── AND ONE 020 FUNCTION: core.get_audit (§1b) ─────────────────────────────
--
-- The shipped web client sends `get_audit('APPROVAL', ref)`, and 020 mapped only
-- `approvals`, so the approval audit tab was empty. §1b adds the one mapping row.
--
-- ── THE RULE, applied identically to all 23 ────────────────────────────────
--
--   * The gate is the FIRST statement of the body. Only the DECLARE
--     initialisers run before it, and every one is a literal or
--     `app.require_tenant_id()`, which reads no table. So no NOT_FOUND,
--     VALIDATION_FAILED or STALE_WRITE can be observed by a caller without the
--     permission, and the refusal is byte-identical for every argument.
--   * READS refuse with `app.err('FORBIDDEN', {requiredPermission})`, the shape
--     `list_quotations` already returns. WRITES refuse with RAISE TRNOS and the
--     same detail. That is 018's read/write refusal rule, unchanged.
--   * `badge_counts` stays open to every role (it is shell bootstrap), but a
--     count is a read: the HRDC and agent-failure counts are 0 without
--     `hrdc:read` / `run:read`. The approvals count was already the caller's
--     own open approvals.
--   * `me` and `navigation` are not gated. `me` answers every principal, and
--     `navigation` already filters by permission.
--
-- WRITERS RETURN THROUGH A READER (`RETURN core.get_proposal(...)`), so a
-- writer's caller also passes the reader's gate. Measured against 002 §11,
-- every holder of `proposal:write`/`proposal:regenerate` holds `proposal:read`,
-- every holder of `quotation:write` holds `quotation:read`, and every holder of
-- `enquiry:edit_extraction` holds `enquiry:read`. `$verify$` V3 asserts those
-- inclusions from the data, so an operator who edits the matrix into a
-- write-without-read state fails this migration instead of getting a
-- successful write followed by a FORBIDDEN body.
--
-- ⚠ ONE CHOICE THE MATRIX DOES NOT MAKE: `get_rate_card`. 002 has no rate-card
-- permission and the fixture checks none. The card carries floor margins and
-- commission bands, and its two readers are the costing worksheet and finance
-- reporting, so it is gated on `quotation:read` (SALES, SALES_MANAGER, FINANCE,
-- MD, ADMIN). Fail closed; reported in the PR as a decision to confirm.
--
-- ⚠ STANDING POSTURE, NOT CHANGED HERE: 014 §4 grants `authenticated` SELECT on
-- every `core` table under a tenant-only policy. These gates are the contract's
-- FORBIDDEN answer on the RPC surface, not a data boundary.
--
-- ⚠ DATA SCOPE (○ in 002 §11) IS NOT APPLIED. SALES holds most reads
-- "scope-narrowed", which 014's policies express through `app.can_see_owner`.
-- A SECURITY DEFINER read does not evaluate the caller's policies, so a SALES
-- caller with `client_scope` MINE still sees every row in the tenant through
...
--
--  1 ENVELOPE. The only new return is `app.err('FORBIDDEN', …)`, and the only new
--    raise is TRNOS with `code: FORBIDDEN`. Both are shapes 018 already returns.
--  2 UNWRAP. No new top-level key.
--  3 RpcMap. Identity arguments are unchanged; V1 asserts one definition each.
--  4 CALL SITES. Unchanged from 018's worked check (rpcClient.ts). R18's only
--    caller is `apps/web/src/features/pipeline/api.ts` through `perform_action`.
--  5 CASTS. No TypeScript.
...
-- ═══ 1 · The permission gates ══════════════════════════════════════════════
--
-- `core.badge_counts`, then the 23 gated RPCs in 018's order of appearance
-- within each group. Each body is 018's, with one `-- 021 ·` block.
...
BEGIN
  ...
  SELECT pg_catalog.count(*)::integer INTO v_hrdc
   ...
   WHERE packet.tenant_id = v_tenant
     ...
     -- 021 · a count is a read: zero for a caller without hrdc:read.
     AND app.has_permission('hrdc:read');
  ...
  SELECT pg_catalog.count(*)::integer INTO v_failures
   ...
   WHERE run.tenant_id = v_tenant
     ...
     -- 021 · zero for a caller without run:read.
     AND app.has_permission('run:read');
...
-- ── 1b · get_audit: the web's own spelling reaches the approvals branch ────
--
-- DEFECT. 020 mapped the URL segment `approvals` onto the approval trail. The
-- web client that shipped (PR #38, `rpcClient.ts` `audit()`) does not send the
-- segment: it sends `aggregateTypeOf('approvals')`, which is `APPROVAL`. 020
-- passes an UPPER_SNAKE value through unchanged, so `APPROVAL` matched no
-- subject and never took the correlated-events branch. The approval audit tab
-- stayed empty against 020.
--
-- The same rule turns `enquiries` into ENQUIRIE and `opportunities` into
-- OPPORTUNITIE, which match nothing either.
--
...
BEGIN
  ...
  SELECT segment.subject_type INTO v_subject
    FROM (VALUES
            ...
            -- 021 · what the shipped client sends. `aggregateTypeOf` in rpcClient.ts
            -- strips one trailing `s` and upper-cases, so `approvals` arrives as
            -- APPROVAL and the two `-ies` plurals arrive misspelt. Each is mapped to
            -- the subject type 012 actually stores; every other output of that rule
            -- (PROPOSAL, QUOTATION, ORGANISATION, …) is already the stored spelling.
...
BEGIN
  ...
  IF NOT app.has_permission('enquiry:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
...
DECLARE
  ...
  v_name    text;
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read, validation or write.
  IF NOT app.has_permission('enquiry:edit_extraction') THEN
    RAISE EXCEPTION 'requester lacks %', 'enquiry:edit_extraction'
      USING ERRCODE = 'TRNOS',
            DETAIL = pg_catalog.jsonb_build_object(
                       'code','FORBIDDEN','requiredPermission','enquiry:edit_extraction')::text;
  END IF;
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read or validation, so the refusal
  -- is identical for every argument.
  IF NOT app.has_permission('contact:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read, validation or write.
  IF NOT app.has_permission('proposal:write') THEN
    RAISE EXCEPTION 'requester lacks %', 'proposal:write'
      USING ERRCODE = 'TRNOS',
            DETAIL = pg_catalog.jsonb_build_object(
                       'code','FORBIDDEN','requiredPermission','proposal:write')::text;
  END IF;
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read or validation, so the refusal
  -- is identical for every argument.
  IF NOT app.has_permission('proposal:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read, validation or write.
  IF NOT app.has_permission('proposal:regenerate') THEN
    RAISE EXCEPTION 'requester lacks %', 'proposal:regenerate'
      USING ERRCODE = 'TRNOS',
            DETAIL = pg_catalog.jsonb_build_object(
                       'code','FORBIDDEN','requiredPermission','proposal:regenerate')::text;
  END IF;
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read, validation or write.
  IF NOT app.has_permission('quotation:write') THEN
    RAISE EXCEPTION 'requester lacks %', 'quotation:write'
      USING ERRCODE = 'TRNOS',
            DETAIL = pg_catalog.jsonb_build_object(
                       'code','FORBIDDEN','requiredPermission','quotation:write')::text;
  END IF;
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read or validation, so the refusal
  -- is identical for every argument.
  IF NOT app.has_permission('policy:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read or validation, so the refusal
  -- is identical for every argument.
  IF NOT app.has_permission('pipeline:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
...
BEGIN
  -- 021 · AUTHZ FIRST: decided before any read or validation, so the refusal
  -- is identical for every argument.
  IF NOT app.has_permission('programme:read') THEN
    RETURN app.err('FORBIDDEN', pg_catalog.jsonb_build_object(
      'requiredPermission','programme:read'));
  END IF;
...
-- state does not rest on 018's name-matched loop.
  ...
  FOR v_fn IN
    SELECT p.oid::regprocedure FROM pg_catalog.pg_proc AS p
     WHERE p.pronamespace = 'core'::regnamespace
       AND p.proname IN ('badge_counts','get_audit','list_enquiries','get_enquiry','patch_enquiry_extraction',
         'list_follow_ups','get_follow_up_draft','get_organisation','get_opportunity','get_contact',
         'get_tna','get_tna_recommendations','create_proposal','list_proposals','get_proposal',
         'add_proposal_section','put_proposal_section','regenerate_proposal_section','get_quotation',
         'put_quotation','get_rate_card','get_policy','get_pipeline_config','get_programme',
         'get_compliance_rule')
  LOOP
...
DECLARE
  ...
  v_pos  integer;
BEGIN
  -- V1 · one definition each, and the 018 posture.
  SELECT pg_catalog.array_agg(g.name ORDER BY g.name) INTO v_bad
    FROM pg_catalog.unnest(ARRAY['badge_counts','get_audit','list_enquiries','get_enquiry','patch_enquiry_extraction',
         'list_follow_ups','get_follow_up_draft','get_organisation','get_opportunity','get_contact',
         'get_tna','get_tna_recommendations','create_proposal','list_proposals','get_proposal',
         'add_proposal_section','put_proposal_section','regenerate_proposal_section','get_quotation',
         'put_quotation','get_rate_card','get_policy','get_pipeline_config','get_programme',
         'get_compliance_rule']) AS g(name)
  ...
  IF v_bad IS NOT NULL THEN
    RAISE EXCEPTION '021 verify V1: overload count or posture wrong on %', v_bad;
  END IF;

  -- V2 · EVERY client-callable RPC in the 018 golden path now checks a
  -- permission, except the three that must not: `me` (answers everyone),
  -- `navigation` (filters by permission) and `badge_counts` (per-count).
  -- The gate is the first permission-relevant statement: no `FROM core.` read
  -- precedes it.
  v_bad := NULL;
  FOR v_gate IN
    SELECT p.oid, p.proname FROM pg_catalog.pg_proc AS p
     WHERE p.pronamespace = 'core'::regnamespace
       AND p.proname IN ('list_enquiries','get_enquiry','patch_enquiry_extraction','list_follow_ups',
         ...
         'add_proposal_section','put_proposal_section','regenerate_proposal_section','get_quotation',
         'put_quotation','get_rate_card','get_policy','get_pipeline_config','get_programme',
         'get_compliance_rule','list_quotations','list_approvals','get_approval','get_audit')
  LOOP
  ...
  END LOOP;
  IF v_bad IS NOT NULL THEN
    RAISE EXCEPTION '021 verify V2: no permission gate, or a read before it, in %', v_bad;
  END IF;

  -- V2b · the shipped client's audit spelling reaches the approvals branch.
  IF pg_catalog.strpos(app._body_sql('core.get_audit(text,text)'::regprocedure), '(''APPROVAL'',') = 0 THEN
    RAISE EXCEPTION '021 verify V2b: core.get_audit does not map APPROVAL';
  END IF;

  -- V3 · WRITERS RETURN THROUGH READERS: every role holding the write holds the
  -- read, read from the matrix as it stands in THIS database.
  SELECT pg_catalog.array_agg(pg_catalog.format('%s holds %s but not %s', w.role, pair.w, pair.r)) INTO v_bad
    FROM (VALUES ('proposal:write','proposal:read'), ('proposal:regenerate','proposal:read'),
                 ('quotation:write','quotation:read'), ('enquiry:edit_extraction','enquiry:read'))
         AS pair(w, r)
  ...
  IF v_bad IS NOT NULL THEN
    RAISE EXCEPTION '021 verify V3: a writer would be refused by its own reader: %', v_bad;
  END IF;
