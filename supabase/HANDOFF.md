...
| `core.hrdc_deadline_status` | `ON_TRACK · AT_RISK · BLOCKED · SUBMITTED` | 003 (type) + 009 compliance/hrdc. Note the spelling: §9 filters this by `AT_RISK`, NOT the `DEADLINE_AT_RISK` that `core.hrdc_packet_panel_state` uses. Two enums on two objects, deliberately |
