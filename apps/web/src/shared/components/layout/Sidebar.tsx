 ...
 * other plane.
 *
 * Top to bottom: the wordmark, then WHO YOU ARE with a light/dark switch beside
 * it, then the navigation, then the application's own affordances in the
 * footer. Identity moved up here from the footer because it is the first thing
 * a reader checks and was in the last place they would look.
 *
          ...
          <div key={group.caption} className="flex flex-col gap-0.5">
            ...
            {group.parents.map((parent) => {
              ...
              /* A leaf parent IS the destination, so it can be selected. A
                 parent with children is only ever a disclosure. */
                ...
                <div key={parent.key} className="flex flex-col">
                  {leaf ? (
                    ...
                    <button
                      ...
                      className={cn(ROW_BASE, "h-8", ROW_IDLE, FOCUS_RING)}
                    >
                      <span aria-hidden="true" className="w-4 shrink-0 text-center">
                        {parent.icon}
                      </span>
                      ...
                      <span
                        aria-hidden="true"
                        className={cn("shrink-0 text-ink-muted", parent.badge ? "ml-1" : "ml-auto")}
                      >
                        {open ? "–" : "+"}
                      </span>
                    </button>
