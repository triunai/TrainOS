...
## Batch 010 — 2026-09-13

| Finding | Severity | State | Applied in | Evidence |
|---|---|---|---|---|
| `C-11` — the invoice model has nowhere to put a MyInvois document | CRITICAL | **Closed at the schema; ONE HALF STILL OPEN** | 010 | Every field the finding names by name now exists and is constrained. `einvoice_uuid` is separate from `einvoice_submission_uid` — the finding's first point, and the two are not interchangeable because only one appears on a QR code. `einvoice_long_id` carries the QR token. `core.einvoice_status` spans NOT_SUBMITTED / SUBMITTED_PENDING_VALIDATION / VALID / INVALID / CANCELLED / REJECTED, which doc 01's four-value `sync_state` could not express. `einvoice_validation_errors` is structured with a key-presence CHECK (R-JSONB). `void_reason` exists, and the **72-hour cancellation window** is a stored deadline enforced by trigger — `test_010` T7a/T7b prove it at 71 and 73 hours, T8 proves it cannot be extended by writing to it. `einvoice_validated_at`, supplier TIN/MSIC/SST registration (`core.tenant_tax_profiles`, because the finding correctly counted `public.tenants` at eight columns), buyer TIN/BRN with `tax_identifier_kind` and a structured address on `core.organisations`, per-line `classification_code` and `uom_code`, `einvoice_type_code`, `exchange_rate`, `is_self_billed` and `is_consolidated` all present. **Credit notes exist** (`H-19`), which is what gives a frozen validated invoice a legal exit. **What is NOT closed:** the finding's first sentence asks for *written confirmation that the accounting package is the submitter of record and round-trips these fields*. That is a client answer, not a schema change. Registered **D-45**, owner Finance, blocking before any real e-invoice traffic. |
| `H-19` — no credit note | HIGH | **Closed** | 010 | `core.credit_notes` + `core.credit_note_lines`, `ON DELETE RESTRICT` against the invoice so a credited invoice cannot be deleted from under the note that credits it, and a DEFERRABLE constraint trigger refusing a credit that exceeds its invoice (`test_010` T12). |
| `C-04` residue — grant sequencing | CRITICAL (residue) | **Carried, flagged not re-derived** | 010 header → 011 | Nothing in 010 grants anything to any client role; `test_010` T15 asserts it per table and per function. The residue is a note in 010's header for 011 to honour. |
| `R-JSONB` on finance jsonb | — | **Applied** | 010 | `einvoice_validation_errors` asserts `? 'errors'` AND `jsonb_typeof(... ) = 'array'`. `test_010` T13c feeds it the exact payload the naive constraint lets past — an object with no `errors` key at all — and it is refused. |
| — | — | **New, found by execution** | 010 | `invoices_void_needs_reason` was written `(voided_at IS NULL) = (void_reason IS NULL)` and was WRONG: it conflated TrainOS voiding its own invoice with the DOCUMENT being cancelled downstream, and since a cancellation sets `void_reason` without `voided_at`, the CHECK made the statutory cancellation path unwalkable. `test_010` T7a caught it. Recorded because the wrong version was the more restrictive of the two and looked the more careful — which is how a wrong constraint survives review. |
| — | — | **New, found by execution** | 010 | `einvoice_cancel_deadline_at` could not be a generated column: PostgreSQL refuses `timestamptz + interval` in a generation expression because it is STABLE, not IMMUTABLE (interval arithmetic depends on the session TimeZone). Downgraded to an unconditional BEFORE trigger and the lost guarantee bought back by `test_010` T8, which writes a deliberately extended deadline and checks the row afterwards. |
